...
async function getMeetingsWithRecordings() {
  try {
    const meetings = [];
    let cursor = null;
    const maxPages = 10; // Reduced: date filter makes this much more efficient
    let pageCount = 0;
    
    const start2026 = new Date('2026-01-01T00:00:00Z').getTime();
    const now = Date.now();
    
    // Use date filter to only get 2026 meetings
    const starts_from = '2026-01-01T00:00:00Z';
    
    do {
      const url = cursor 
        ? `https://api.attio.com/v2/meetings?limit=200&starts_from=${encodeURIComponent(starts_from)}&cursor=${encodeURIComponent(cursor)}`
        : `https://api.attio.com/v2/meetings?limit=200&starts_from=${encodeURIComponent(starts_from)}`;
            ...
            for (const recording of recordings) {
              if (recording.created_at) {
                ...
                if (recordingTime >= start2026 && recordingTime <= now) {
                  ...
                  console.log(`      ✅ Found 2026 recording: ${meeting.title || 'Untitled'} (${new Date(recording.created_at).toLocaleDateString()})`);
                }
              }
            }
          } else {
            // Log HTTP errors (401, 404, etc.)
            if (recordingsResponse.status !== 404) {
              console.log(`      ⚠️  Failed to get recordings for meeting: ${recordingsResponse.status}`);
            }
          }
        } catch (error) {
          console.error(`      ❌ Error checking meeting: ${error.message}`);
          continue;
        }
      }
...
// Check if running directly (works cross-platform)
const isMain = import.meta.url.endsWith(process.argv[1].replace(/\\/g, '/'));

if (isMain || process.argv[1]?.includes('polling-service')) {
  // Running directly (node src/polling-service.js)
  console.log('\n🧪 Running polling service in test mode...\n');
  ...
  pollForNewRecordings()
    ...
    .catch((err) => {
      console.error('\n❌ Polling test failed:', err.message);
